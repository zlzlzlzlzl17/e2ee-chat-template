...
## 12. Updating This Template Later

Before updating:

1. Back up the database and uploaded files.
2. Review local changes to `.env`, reverse-proxy config, and Android config.
3. Test on a staging environment if possible.

Because this repository has already moved away from the old `familychat` naming and legacy key-prefix scheme, treat future updates as template maintenance, not legacy-format migration.
